%%%In this file whe make a database where to store the users -> seperation of concerns
...
%server_actor temprarily stores all users it will immediately create a database process.
%whenever server_actor needs data it's going to communicate with the database.
%this will bring us efficiency gains

server_actor(Users) ->
    DatabasePid = spawn_link(dataBase,[Users]),
    server_actor(DatabasePid), 
    receive
        %testing purposes
        {Sender, users} ->
...
bilal_initialize_test() ->
    ...
    ulb ! {self(), users},
    receive 
        {_, users, Users}  -> ?assertMatch(Users,UsersUlb);
        _ -> erlang:error(unexpected_message_received) end,
    vub ! {self(), users},
    receive 
        {_, users, Users2} -> ?assertMatch(Users2,UsersVub);
         _ -> erlang:error(unexpected_message_received) end,
    ?assertMatch(ok,ok).
